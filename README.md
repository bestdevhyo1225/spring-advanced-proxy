...
| Proxy (개발자가 인터페이스를 직접 구현한 프록시) | Target | | :-: | :-: | | `OrderControllerV1Proxy(프록시 객체)` ->
| `OrderControllerV1Impl(실제 객체)` -> | | `OrderServiceV1Proxy(프록시 객체)` -> | `OrderServiceV1Impl(실제 객체)` -> |
...
`바이트 코드를 조작` 해서 동적으로 클래스를 생성하는 기술을 제공한다. CGLIB을 사용하면, 인터페이스가 없어도 구체 클래스만 가지고 동적 프록시를 생성할 수 있다.
