...
예를 들면, `LogTrace`, `TimeTrace` 와 같은 `어드바이저(하나의 포인트컷 + 하나의 어드바이스)` 들이 있을 때, 프록시 객체는 2개가 생성되는 것이 아니고, **`하나만 생성된다.`** 생성된 하나의 프록시 객체가
`LogTrace`, `TimeTrace` 를 차례대로 호출하고, 마지막에 `Target` 을 호출하는 흐름으로 이해하면 된다.
